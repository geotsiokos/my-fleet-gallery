<?php
/**
 * Plugin Name: Modern Fleet Gallery Pro (Global Support)
 * Description: Moves WhatsApp to the Index Card and uses a global phone number.
 * Version: 17.1
 */

if ( ! defined( 'ABSPATH' ) ) exit;

// 1. Post Type & Meta (Cleaned up - WhatsApp removed from individual vehicles)
add_action( 'init', function() {
	...
	add_action('add_meta_boxes', function() {
		...
		/* Global Support Link in Index Card */
		...
		.index-wa-support:hover .wa-icon { background: white; color: #25D366; }
		...
		.cta-group { margin-top: auto; }
		.btn-book { display: block; width: 100%; text-align: center; padding: 14px; border-radius: 12px; font-weight: 700; font-size: 0.95rem; text-decoration: none !important; background: #1e272e; color: #fff !important; transition: 0.2s; }
	...
	");
				...
				add_shortcode( 'fleet_gallery', function( $atts ) {
					...
					$a = shortcode_atts( array(
							...
							'wa_number' => '1234567890', // Default number if none provided
					...
					$wa_html = '<a href="https://wa.me/'.$clean_wa.'" class="index-wa-support" target="_blank">
					<i class="wa-icon">w</i>
					<span>'.esc_html($a['wa_text']).'</span>
				</a>';
